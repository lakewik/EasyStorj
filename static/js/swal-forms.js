    ...
    getFormValues: function (isConfirm) {
      ...
      function toValuableAttrs (tag) {
        ...
        if (isConfirm && tag.dataset.swalFormsRequired && !tag.value) {
          var warnMsg = 'Please fill out all required fields!';
          //+ (tag.name || tag.id)
          ...
          //document.querySelector('.swal-form').append(warnMsg)
    ...
    var input = {
      ...
      toHtml: function () {
        ...
        if (input.type !== 'select') {
          inputTag = t("<label class='control-label''><b>{placeholder}:</b></label> <input id='{id}' class=' {clazz} form-control boxed swal-form-field' type='{type}' name='{name}'" +
            " value='{value}' title='{placeholder}' placeholder=''" +
            ' data-swal-forms-required={required}><br>', input)
        } else {
          inputTag = t("<label class='control-label''><b>{placeholder}:</b></label><select id='{id}' class='{clazz} swal-form-field nice-input' name='{name}'" +
