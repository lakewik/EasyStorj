...
class NodeDetailsUI(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None, nodeid=None):
        ...
        # close window
        QtCore.QObject.connect(self.node_details_ui.ok_bt,
                               QtCore.SIGNAL('clicked()'), self.close)
        self.connect(self, QtCore.SIGNAL('showBridgeExceptionMessageBox'),
    ...
    def show_storj_bridge_exception(self, exception_content):
        try:
            j = json.loads(exception_content)
            QtGui.QMessageBox.critical(self, 'Bridge error', str(j['error']))
        except Exception as e:
            self.__logger.error(e)
            QtGui.QMessageBox.critical(self, 'Bridge error', exception_content)

    def createNewNodeDetailsResolveThread(self):
        download_thread = threading.Thread(target=self.initialize_node_details,
                                           args=())
    ...
    def initialize_node_details(self):
        ...
        try:
            self.node_details_content = \
                self.storj_engine.storj_client.contact_lookup(self.nodeid)

            # get given node address
            self.node_details_ui.address_label.setText(
                self.node_details_content.address)
            # get last timeout
            ...
            self.node_details_ui.user_agent_label.setText(
                self.node_details_content.userAgent)
            ...
            self.node_details_ui.response_time_label.setText(
                str(self.node_details_content.responseTime))
            ...
            ip_addr = socket.gethostbyname(self.node_details_content.address)
            ...
            self.node_details_ui.country_label.setText(
                country_full_name)  # Set full country name
