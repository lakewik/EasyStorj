# -*- coding: utf-8 -*-
"""
Module with cryptographic functions for Storj GUI Client.

Based on: <http://bit.ly/2pmaImR>
"""
...
class FileCrypto:
    ...
    def encrypt_file(self, algorithm, file_path, encrypted_file_save_path, password):
        if algorithm == 'AES':
            with open(file_path, 'rb') as in_file, \
                    open(encrypted_file_save_path, 'wb') as out_file:
    ...
    def decrypt_file(self, algorithm, file_path, decrypted_file_save_path, password):
        if algorithm == 'AES':
            with open(file_path, 'rb') as in_file, \
                    open(decrypted_file_save_path, 'wb') as out_file:
