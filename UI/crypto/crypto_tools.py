...
from storj.model import Keyring


class CryptoTools:

    __logger = logging.getLogger('%s.CryptoTools' % __name__)
    ...
    def prepare_bucket_entry_hmac(self, shard_array):
        encryption_key = Keyring().get_encryption_key('test')
        current_hmac = ''
        for shard in shard_array:
            base64_decoded = '%s%s' % (str(base64.decodestring(shard.hash)), str(current_hmac))
