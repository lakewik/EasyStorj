...
class RegisterUI(QtGui.QMainWindow):
    ...
    def register(self):
        ...
        if self.email != '' and self.password != '' \
                and self.password_repeat != '':
            if self.password == self.password_repeat:
                if (self.tools.check_email(self.email)):
                    # Take login action
                    try:
                        self.storj_client = storj.Client(None, '').\
                            user_create(str(self.email).strip(),
                        ...
                        if j[0]['error'] == 'Email is already registered':
                            QtGui.QMessageBox.about(self, 'Warning',
                                                    'User with this e-mail \
is already registered! Please login or try a different e-mail!')
                        else:
                    ...
                    QtGui.QMessageBox.about(self, 'Warning',
                                            'Your e-mail seems to be invalid! \
Please check e-mail and try again')
            else:
                QtGui.QMessageBox.about(self, 'Warning',
...
Please check and try again!')
        ...
        if success:
            msgBox = QtGui.QMessageBox(QtGui.QMessageBox.Information,
                                       'Success',
                                       'Successfully registered in Storj \
Distributed Storage Network! '
                                       'Now, you must verify your email by'
                                       'clicking the link that has been \
sent to you. '
                                       'Then you can login',
