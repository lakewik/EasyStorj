...
from .resources.constants import MINIMAL_ALLOWED_BRIDGE_REQUEST_TIMEOUT, \
...
class ClientConfigurationUI(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        QtCore.QObject.connect(self.client_configuration_ui.apply_bt,
                               QtCore.SIGNAL('clicked()'),
                               self.save_settings)

        # close form
        QtCore.QObject.connect(self.client_configuration_ui.cancel_bt,
                               QtCore.SIGNAL('clicked()'),
                               self.close)
        ...
        # logout
        QtCore.QObject.connect(self.client_configuration_ui.logout_bt,
                               QtCore.SIGNAL('clicked()'),
                               self.handle_logout_action)

        # clear logs
        QtCore.QObject.connect(self.client_configuration_ui.clear_logs_bt,
                               QtCore.SIGNAL('clicked()'),
        ...
        self.client_configuration_ui.bridge_request_timeout.setMinimum(
            MINIMAL_ALLOWED_BRIDGE_REQUEST_TIMEOUT)

        self.client_configuration_ui.bridge_request_timeout.setValue(
            DEFAULT_MAX_BRIDGE_REQUEST_TIMEOUT)

        self.client_configuration_ui.max_shard_size_enabled_checkBox.\
            stateChanged.connect(
                lambda x: self.enable_shard_size_inputs() if x
                else self.disable_shard_size_inputs())

        self.configuration_manager.paint_config_to_ui(
            self.client_configuration_ui)

        if self.client_configuration_ui.max_shard_size_enabled_checkBox.\
                isChecked():
    ...
    def select_crypto_keys_path(self):
        self.client_configuration_ui.crypto_keys_location.setText(
            str(QtGui.QFileDialog.getSaveFileName(
                self, 'Save file to...', 'storj_client_keyring.sjkr')))

    def save_settings(self):
        # validate settings
        self.configuration_manager.save_client_configuration(
            self.client_configuration_ui)  # save configuration
        QtGui.QMessageBox.about(self, 'Success',
    ...
    def reset_settings_to_default(self):
        ...
        if result == QtGui.QMessageBox.Yes:
            QtGui.QMessageBox.about(
                self, 'Success',
                'Settings successfully restored to default!')
