...
class FileMirrorsListUI(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None, bucketid=None, fileid=None):
        ...
        #       QtCore.SIGNAL("selectionChanged(QItemSelection, QItemSelection)"), self.open_mirror_details_window)

        # self.connect(self.file_mirrors_list_ui.established_mirrors_tree,
        #   QtCore.SIGNAL('selectionChanged()'), self.open_mirror_details_window)

        # QtCore.QObject.connect(self.file_mirrors_list_ui.established_mirrors_tree.selectionModel(),
    ...
    def initialize_mirrors_tree(self):
        ...
        # set loading color
        # self.file_mirrors_list_ui.loading_label_mirrors_established.setStyleSheet('color: red')
        # self.file_mirrors_list_ui.loading_label_mirrors_available.setStyleSheet('color: red')
        ...
        try:
            for file_mirror in self.storj_engine.storj_client.file_mirrors(
                    str(self.bucketid), str(self.fileid)):
                ...
                for mirror in file_mirror.established:
                    ...
                    child1 = QtGui.QStandardItem('%s:%d' % (mirror['contact']['address'], mirror['contact']['port']))
                    ...
                    # self.established_mirrors_tree_view.setFirstColumnSpanned(
                    #   1, self.established_mirrors_tree_view.rootIndex(), True)
            ...
            for file_mirror in self.storj_engine.storj_client.file_mirrors(str(self.bucketid), str(self.fileid)):
                ...
                for mirror_2 in file_mirror.available:
                    ...
                    child1 = QtGui.QStandardItem(
                        '%s:%d' % (mirror_2['contact']['address'], mirror_2['contact']['port']))
            ...
            self.__logger.error(e)
            self.emit(QtCore.SIGNAL('showUnhandledException'), str(e))
            logger.error(e)
